...
const EditTodo = ({ user }) => {
  ...
  const [p_first_name, setPName] = useState("");
  ...
  async function getPName() {
    try {
      const response = await fetch(`http://localhost:5001/auth/searchpatient/?username=${user.username}`, {
        method: "GET",
