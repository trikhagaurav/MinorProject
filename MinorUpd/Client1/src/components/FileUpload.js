...
const FileUpload = () =>
{
    ...
    const [statusFile, setStatusFile] = useState("Please upload your Doctor's License/Degree certificate");
    const [imgPreview, setImagePreview] = useState("Please upload your Doctor's License/Degree certificate");
    ...
    const onSubmitForm = async (e) => 
    {
        ...
        try
        {
            const res = await axios.post("http://localhost:5001/auth/upload", formData, { headers : {'token' : localStorage.token}})
            ...
            if(res.status===200&&res.data.msg===undefined)
            {
                ...
                setImagePreview('Your License/Degree is successfully submitted for verificatino')
